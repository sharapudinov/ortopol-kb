...
| `pg_search.py` | поиск |
| `pg_schema_citation.sql`, `pg_graph.py`, `pg_graph_candidates.py` | граф цитирований: схема, AGE-проекция (`citation_graph`), потребители (`citers`/`candidates`/`cocitation`/`hybrid`) |
...
999 тестов, без Docker и без сети; тестам, которым нужна живая база, при её
