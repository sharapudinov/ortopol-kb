...
Usage:
    python3 pg_load.py [--corpus-dir DIR] [--pgenv FILE]
"""
...
DOCUMENT_COLUMNS = "corpus.documents (id, filename, extraction_state, source_tier, pages_count, chars_extracted, note)"
...
def _documents_csv(manifest: dict) -> str:
    buf = io.StringIO()
...
INSERT INTO corpus.documents (id, filename, extraction_state, source_tier,
                              pages_count, chars_extracted, note)
SELECT id, filename, extraction_state, source_tier, pages_count, chars_extracted, note
...
def load_corpus(corpus_dir: Path, pgenv_path: Path) -> dict:
    ...
    copy_csv_into(env, "corpus.stage_documents (id, filename, extraction_state, "
                       "source_tier, pages_count, chars_extracted, note)",
                  _documents_csv(manifest))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = load_corpus(args.corpus_dir, pgenv_path)
